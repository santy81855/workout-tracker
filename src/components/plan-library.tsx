...
export function PlanLibrary({ showStarter = false }: { showStarter?: boolean }) {
  ...
  return <section className="program-section plan-library" aria-labelledby="plan-library-title">
    ...
    {cycles === null ? <p className="muted-copy list-status">Loading your plans…</p> : cycles.length === 0 && !showStarter ? <p className="muted-copy list-status">No saved plans yet.</p> : <div className="library-list">{cycles.map((cycle) => <article className="library-plan" key={cycle.cycleId}><div><span className={`library-status library-status-${cycle.status}`}>{cycle.status === "planned" ? "Paused" : cycle.status}</span><h3>{cycle.document.displayTitle ?? cycle.document.name}</h3><p>{cycle.document.name} · {cycle.completedSessions} of {cycle.document.weekCount * cycle.document.workoutsPerWeek} sessions</p></div>{cycle.status === "planned" ? <button disabled={pending !== null} onClick={() => resume(cycle)} type="button">{pending === cycle.cycleId ? "Resuming…" : "Resume"}</button> : null}</article>)}</div>}
