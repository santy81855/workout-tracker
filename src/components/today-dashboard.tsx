...
export function TodayDashboard() {
  ...
  function updateRestDayDropTarget(clientX: number, clientY: number) {
    const target = document.elementFromPoint(clientX, clientY)?.closest<HTMLElement>("[data-queue-index]");
    if (!target) return;
    const index = Number(target.dataset.queueIndex);
    const lowerHalfOfLastWorkout = index === upcomingQueue.length - 1
      && clientY > target.getBoundingClientRect().top + target.getBoundingClientRect().height / 2;
    setQueueDropIndex(lowerHalfOfLastWorkout && queueBoundary ? upcomingQueue.length : index);
  }
  ...
  return (
    ...
    {upcomingQueue.length > 1 || recoverableSkip ? <section className="section-block upcoming-workout-queue" aria-labelledby="upcoming-workouts-title">
      ...
      <ol>{upcomingQueue.map((queued, index) => <Fragment key={queued.scheduledWorkoutId}>
        {queued.restDays.map((restDay) => <li className="queue-rest-day" key={restDay.id}><button className="drag-handle" aria-label={`Drag rest day scheduled ${restDay.restDate}`} disabled={activeSession !== null || reordering} onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); draggingRestDay.current = restDay.id; setQueueDropIndex(index); }} onPointerMove={(event) => { if (draggingRestDay.current) updateRestDayDropTarget(event.clientX, event.clientY); }} onPointerUp={() => void finishRestDayDrag()} onPointerCancel={() => void finishRestDayDrag()} type="button">⠿</button><div><strong>Rest day</strong><small>{new Date(`${restDay.restDate}T12:00:00`).toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" })}</small></div><button className="delete-rest-day" aria-label={`Delete rest day scheduled ${restDay.restDate}`} disabled={deletingRest !== null} onClick={() => void deleteRestDay(restDay.id)} type="button">{deletingRest === restDay.id ? "…" : "×"}</button></li>)}
